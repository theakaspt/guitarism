...
// 분할하면서 새로 생긴 선언 (원본에 없던 것). 여기 적힌 것 말고는 늘어나면 안 된다.
...
const DECL = /^(?:export\s+default\s+function|export\s+(?:const|let|var|function|class)|const|let|var|function|class)\s+([A-Za-z_$][\w$]*)/;
...
function namesIn(src) {
  ...
  for (const line of src.split("\n")) {
    ...
    const multi = line.match(/^(?:export\s+)?(?:const|let|var)\s+(.+)$/);
    if (multi && m) {
