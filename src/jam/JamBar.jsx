    ...
    <>
      ...
      {open && (
        <div style={{ position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 60, background: C.panel, borderTop: `1px solid ${C.ring}`, borderRadius: "18px 18px 0 0", maxHeight: "82vh", overflowY: "auto", boxSizing: "border-box" }}>
          <div style={{ maxWidth: 440, margin: "0 auto", padding: "12px 16px 24px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
              ...
              <button onClick={() => setOpen(false)} aria-label="잼 설정 닫기" style={{ marginLeft: "auto", border: "none", background: "transparent", color: C.muted, fontSize: 15, cursor: "pointer", padding: "2px 8px" }}>▾ 닫기</button>
            </div>
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              <div style={{ flex: "0 0 30%", minWidth: 0 }}>
                <div style={{ ...SEGLABEL, marginBottom: 4, textAlign: "center" }}>키</div>
                <VWheel label="잼 키" items={KEYS.map((k) => k.short)} value={jam.keyIdx} onChange={(i) => jam.setKeyIdx(i)} size={19} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ ...SEGLABEL, marginBottom: 4, textAlign: "center" }}>폼</div>
                <VWheel label="잼 폼" items={JAM_FORMS.map((f) => f.name)} value={jam.form} onChange={(i) => jam.setForm(i)} size={15} />
              </div>
            </div>
            ...
            {nowVoicing && (
              <div style={{ marginTop: 12, background: C.bg, borderRadius: 12, padding: "12px 8px 8px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "center", gap: 16, marginTop: 8, flexWrap: "wrap", fontSize: 11, color: C.muted }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ ...SEGLABEL, marginBottom: 4, textAlign: "center" }}>리듬</div>
                <VWheel label="컴핑 리듬" items={COMP_RHYTHMS.map((r) => r.name)} value={jam.rhythm} onChange={(i) => jam.setRhythm(i)} size={13} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ ...SEGLABEL, marginBottom: 4, textAlign: "center" }}>음색</div>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ ...SEGLABEL, marginBottom: 4, textAlign: "center" }}>드럼</div>
      ...
      {jam.audioHint && (
        <div style={{ position: "fixed", left: 0, right: 0, bottom: 108, zIndex: 53, background: C.roseBg, color: C.roseText }}>
          <div style={{ maxWidth: 440, margin: "0 auto", padding: "8px 16px", fontSize: 11, textAlign: "center", boxSizing: "border-box" }}>{jam.audioHint}</div>
        </div>
      )}
      <div style={{ position: "fixed", left: 0, right: 0, bottom: 62, height: 46, zIndex: 52, background: C.panel }}>
        <div onClick={() => setOpen((o) => !o)} role="button" tabIndex={0} aria-expanded={open} aria-label="잼 설정 열기" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setOpen((o) => !o); } }} style={{ maxWidth: 440, margin: "0 auto", height: "100%", display: "flex", alignItems: "center", gap: 12, padding: "0 16px", cursor: "pointer", boxSizing: "border-box" }}>
          ...
          <span style={{ marginLeft: "auto", display: "flex", gap: 12, alignItems: "center", flexShrink: 0 }}>
            <button onClick={(e) => { e.stopPropagation(); jam.setPlaying(!jam.playing); }} aria-pressed={jam.playing} aria-label={jam.playing ? "잼 정지" : "잼 시작"} style={{ width: 32, height: 32, borderRadius: 999, border: `1px solid ${jam.playing ? C.brass : C.ring}`, background: jam.playing ? C.brass : C.hub, color: jam.playing ? C.bg : C.text, cursor: "pointer", fontSize: 13, fontWeight: 800 }}>{jam.playing ? "■" : "▶"}</button>
